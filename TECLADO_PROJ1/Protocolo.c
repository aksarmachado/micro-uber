...
void transmite_estado_veiculo(char status_veiculo)
{
	...
	transmite_string_serial(envia, 3); //transmite mensagem de qual o estado do carro
	
}

// Função para transmitir a mensagem de corrida iniciada por comunicação serial para o servidor externo(recebe o byte contendo o codigo do cliente)
void transmite_inicio_corrida(char cliente){
	char envia[4]={0};			//Vetor que receberá a mensagem de qual movimento o carro terá
		...
		// escreve_string_lcd("SI");			// mensagem para sinalizar inicio de corrida no LCD
		// _delay_ms(200);
		// limpa_display();
	
}

// Função para transmitir a mensagem de fim de corrida por comunicação serial para o servidor externo(recebe o byte contendo o codigo do cliente)
void transmite_final_corrida(char cliente){
	...
	// escreve_string_lcd("SF");				// mensagem para sinalizar fim de corrida no LCD
	// _delay_ms(200);
	// limpa_display();
